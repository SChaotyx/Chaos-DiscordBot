...
async function executeWeekly(interactionOrMessage, authorId, channelId) {
    try {
        const current = Math.floor(Date.now() / 1000);
        
        // Buscar el weekly actual (type = 1)
        const weeklyData = await db.queryOne(
            'SELECT levelID FROM dailyfeatures WHERE timestamp < ? AND type = 1 ORDER BY timestamp DESC LIMIT 1',
            [current]
        );
        ...
        const levelID = weeklyData.levelID;
        const messageData = await buildLevelEmbedCompact(levelID, authorId, `${emojis.icon_weekly} Current Weekly Level`);
        ...
        console.error('Error en comando weekly:', error);
        const errorMsg = '❌ Error al buscar el weekly.';
        if (interactionOrMessage.isCommand?.()) {
            if (interactionOrMessage.deferred || interactionOrMessage.replied) {
                await interactionOrMessage.editReply({ content: errorMsg });
            } else {
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('weekly')
        .setDescription('Muestra el nivel Weekly actual'),
    async execute(interaction) {
        await interaction.deferReply(); // Defer reply since image generation takes time
        await executeWeekly(interaction, interaction.user.id, interaction.channel.id);
    }
};
