...
async function executeIconSet(interaction, userName, includeJetpack) {
    try {
        ...
        if (!userName || userName.trim() === '') {
            // Generate random icon set with all types (including jetpack since includeJetpack is true)
        ...
        console.error('Error en comando iconSetFull:', error);
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('iconsetfull')
        .setDescription('Muestra el conjunto completo de iconos de un usuario (incluye jetpack)')
    ...
    async execute(interaction) {
        ...
        await executeIconSet(interaction, userName, true);
    }
};
